...
class DreamNetwork:
    def __init__(self, network_config):
        ...
        assert (
            self.image_preprocessing() in sgtapose.image_proc.KNOWN_IMAGE_PREPROC_TYPES
        ...
        if self.architecture_type == "vgg":
            ...
            if "n_stages" in self.network_config["architecture"]:
                self.model = torch.nn.DataParallel(
                    sgtapose.models.DreamHourglassMultiStage(
                ...
                self.model = torch.nn.DataParallel(
                    sgtapose.models.DreamHourglass(
                        self.n_keypoints, **vgg_kwargs
                    ),
                    device_ids=data_parallel_device_ids,
                ).cuda()
            ...
            self.model = torch.nn.DataParallel(
                sgtapose.models.ResnetSimple(self.n_keypoints, **resnet_kwargs),
                device_ids=data_parallel_device_ids,
            ).cuda()
    ...
    def net_resolutions_from_image_raw_resolution(
        ...
        net_input_resolution = sgtapose.image_proc.resolution_after_preprocessing(
    ...
    def keypoints_from_image(
        ...
        input_image_preproc_before_norm = sgtapose.image_proc.preprocess_image(
        ...
        detected_kp_projs_net_in = sgtapose.image_proc.convert_keypoints_to_netin_from_netout(
            detected_kp_projs_net_out, netout_res_inf, netin_res_inf
        )
        detected_kp_projs = sgtapose.image_proc.convert_keypoints_to_raw_from_netin(
    ...
    def save_network(
        ...
        sgtapose.utilities.makedirs(output_dir, exist_ok=overwrite)
